...
def stresnet(c_dim, p_dim, t_dim, y_dim, residual_units, filters, day_info_dim):
    main_inputs, outputs = [], []
    ...
    for dim in [c_dim, p_dim, t_dim]:
        ...
        # Conv1
        conv1 = Conv2D(filters=filters, kernel_size=(3, 3), padding='same', data_format='channels_last')(input)
        # Residual Networks
        residual_output = res_units(filters=filters, repetations=residual_units)(conv1)
        # Conv2
    ...
    if day_info_dim > 0:
        ...
        x_day = TimeDistributed(Dense(units=map_height*map_width*nb_channel, activation='linear'))(day_input)
