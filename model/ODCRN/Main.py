...
class ModelTrainer(object):
    ...
    def get_model(self):
        if self.params['model'] == 'ODCRN':
            model = ODCRN.ODCRN(num_nodes=self.params['N'],
                                K=self.K,
                                input_dim=1,
            ...
            for mode in modes:
                ...
                for x_seq, y_true in data_loader[mode]:
                    with torch.set_grad_enabled(mode=(mode=='train')):
                        if self.params['model'] == 'ODCRN':
                            y_pred = self.model(X_seq=x_seq, G=self.G)
                        else:
                            raise NotImplementedError('Invalid model name.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', '--model', type=str, help='Specify model', choices=['ODCRN'], default='ODCRN')
    parser.add_argument('-DGC', '--use_DGC', type=int, default=1)       # 1: OCDRN w/ DGC 0: ODCRN w/o DGC
