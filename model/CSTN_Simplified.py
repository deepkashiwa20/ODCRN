...
# from TimeDistributedPlus import TimeDistributedPlus
from Param import *
...
def CSTN_Simplified(timestep_in, timestep_out, height, width, channel):
    ...
    # temporal_inputs = [od_times]
    # for step_data in zip(*temporal_inputs):
    od_encoded_seq = []
    for step_data in od_times:
        conv1 = Conv2D(filters=32, kernel_size=(3, 3), padding='same')(step_data)
    ...
    ft = ConvLSTM2D(filters=32, kernel_size=(3,3), padding='same', return_sequences=False)(od_encoded)
    ft = Conv2D(filters=32, kernel_size=(1, 1), padding='same', activation='relu')(ft)
    
    fusion0 = Conv2D(filters=32, kernel_size=(1, 1), padding='same', activation='relu')(ft)
    fusion0 = Reshape((-1, 32))(fusion0)
    fusion0 = Permute((2, 1))(fusion0)
    fusion1 = Conv2D(filters=32, kernel_size=(1, 1), padding='same', activation='relu')(ft)
    fusion1 = Reshape((-1, 32))(fusion1)
    fusion1 = Permute((2, 1))(fusion1)
    fusion2 = Conv2D(filters=32, kernel_size=(1, 1), padding='same', activation='relu')(ft)
    fusion2 = Reshape((-1, 32))(fusion2)
    ...
    output = Conv2D(1, kernel_size=(1, 1), padding='same')(softmaxfusion)
    output = Activation('relu')(output)
    model = Model(inputs=od_input, outputs=output)
    return model

# def main():
#     model = CSTN_Simplified(TIMESTEP_IN, TIMESTEP_OUT, HEIGHT, WIDTH, CHANNEL)
