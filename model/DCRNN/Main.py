...
class ModelTrainer(object):
    ...
    def get_model(self):
        if self.params['model'] == 'DCRNN':
            model = DCRNN.DCRNN(num_nodes=self.params['N'],
            ...
            for mode in modes:
                ...
                for x_seq, y_true in data_loader[mode]:
                    with torch.set_grad_enabled(mode=(mode=='train')):
                        if self.params['model'] == 'DCRNN':
                            y_pred = self.model(x_seq=x_seq.squeeze(dim=-1), P=self.G).unsqueeze(dim=-1)
                        else:
                            raise NotImplementedError('Invalid model name.')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run OD Prediction with DCRNN')

    # command line arguments
    parser.add_argument('-GPU', '--GPU', type=str, help='Specify GPU usage', default='cuda:3')
    parser.add_argument('-in', '--input_dir', type=str, default='../../data')
    parser.add_argument('-out', '--output_dir', type=str, default='./output')
    parser.add_argument('-model', '--model', type=str, help='Specify model', choices=['DCRNN'], default='DCRNN')
